...
def image_to_BGR(image: Image.Image) -> Tuple[bytes, int]:  # BGR is a proper name
    if image.mode not in ['RGB', 'RGBA']:
        # we need the first 3 channels to be R, G and B
        image = image.convert('RGB')
    rgb = np.asarray(image)
...
def image_to_BGRA(image: Image.Image) -> Tuple[bytes, int]:  # BGRA is a proper name
    if image.mode != 'RGBA':
...
def image_to_compressed_BGRA(image: Image.Image) -> Tuple[bytes, int]:
    ...
    for h in range(image.height):
        for w in range(image.width):
            ...
            pixel = image_data[w, h] # type: ignore
            a = pixel[3] >> 4
