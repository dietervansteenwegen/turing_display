...
class LcdBase(ABC):
    ...
    def display_progress_bar(
        ...
        background_color = parse_color(background_color)
        self._check_position_and_size(x=x, y=y, width=width, height=height)
    ...
    def display_line_graph(  # noqa: C901
        self,
        x: int,
        y: int,
        width: int,
        ...
        if autoscale:
            ...
            if true_min != max_value and true_max != min_value:
        ...
        y_scale = height / (max_value - min_value)
        ...
        draw.line(list(zip(plots_x, plots_y, strict=True)), fill=line_color, width=line_width)
    ...
    def draw_radial_decoration(
        ...
        if math.modf(x_f) == 0.5:
            x_f = math.floor(x_f) if i_cos > 0 else math.ceil(x_f)
        else:
            x_f = math.floor(x_f + 0.5)

        y_f = (i_sin * (radius - width / 2)) + radius
        if math.modf(y_f) == 0.5:
            y_f = math.floor(y_f) if i_sin > 0 else math.ceil(y_f)
        else:
    ...
    def display_radial_progress_bar(  # noqa: C901
        self,
        xc: int,
        yc: int,
        radius: int,
        ...
        if angle_start % 361 == angle_end % 361:
            if clockwise:
                ...
                angle_end += 0.1
        # TODO: Get rid of asserts here
        assert xc - radius >= 0 and xc + radius <= self.get_width(), (
            'Radial is out of screen (left/right)'
        )
        assert yc - radius >= 0 and yc + radius <= self.get_height(), (
            'Radial is out of screen (up/down)'
        )
        assert 0 < bar_width <= radius, (
            f'Radial linewidth is {bar_width}, must be > 0 and <= radius'
        )
        assert angle_end % 361 != angle_start % 361, (
        ...
        assert angle_sep * angle_steps < 360, (
        ...
        if clockwise:
            ...
            # discontinued bar case
                ...
                etapes = int((angle_e - angle_start) / angle_complet)
