...
# for USB-C displays like Turing Smart Screen or XuanFang
...
class Command(IntEnum):
    ...
    DISPLAY_PIXELS = 195  # Displays a list of pixels than can be non-contiguous in one command,
...
class LcdCommRevA(LcdBase):
    ...
    def send_command(
        ...
        # If no queue for async requests, or if asked explicitly to
        # do the request sequentially: do request now
    ...
    def _hello(self) -> None:
        ...
        log.debug(f'HW sub-revision: {self.sub_revision.name}')
    ...
    def set_brightness(self, level: int = 25) -> None:
        if 0 <= level <= 100:
            ...
            log.error('Brightness level must be [0-100], ignoring command')
    ...
    def display_pil_image(
        ...
        self._check_position_and_size(x, y, image_width, image_height)
