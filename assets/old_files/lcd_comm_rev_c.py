...
from library.lcd.serialize import chunked, image_to_BGR, image_to_BGRA
...
class LcdCommRevC(LcdComm):
    ...
    def _hello(self):
        ...
        response = ''.join(
            filter(
                lambda x: x in set(string.printable),
        ...
        if self.display_width == 480 and self.display_height == 480:
            ...
            logger.error(
                f'Unsupported resolution {self.display_width}x{self.display_height} for revision C'
            )
        ...
        try:
            ...
            if self.rom_version < 80 or self.rom_version > 100:
                logger.warning(
                    'ROM version %d may be invalid, use default ROM version 87' % self.rom_version
                )
                self.rom_version = 87
        except:
        ...
        logger.debug(
            ...
            % ((str(self.sub_revision)), self.rom_version)
        )
    ...
    def SetBrightness(self, level: int = 25):
        ...
        self._send_command(
            Command.SET_BRIGHTNESS, payload=bytearray((converted_level,)), bypass_queue=True
        )
